...
class OnPolicyRunner:
    ...
    def get_inference_estimator(self, unnorm: bool = True, device: str | None = None) -> Callable:
        if not self.alg_cfg["estimator"]:
            raise ValueError("No estimator configuration found.")
        self.eval_mode()  # switch to evaluation mode (dropout for example)
        if device is not None:
            self.alg.policy.to(device)
        return lambda obs, unnorm=unnorm: self.alg.policy.estimate_inference(obs, unnorm)
    ...
    def _construct_algorithm(self, obs: TensorDict) -> PPO:
        ...
        # resolve estimator config
        self.alg_cfg, self.use_last_obs = resolve_estimator_config(self.alg_cfg)
