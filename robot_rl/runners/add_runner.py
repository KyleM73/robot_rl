...
import robot_rl
from robot_rl.algorithms import ADD
from robot_rl.env import VecEnv
from robot_rl.modules import ActorCriticDiscriminator, resolve_rnd_config, resolve_symmetry_config
from robot_rl.utils import resolve_obs_groups, store_code_state


class OnPolicyADDRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        default_sets = ["critic", "discriminator"]
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    rewards = self.alg.policy.disc_rewards(obs).flatten()
                    # Move to device
    ...
    def _construct_algorithm(self, obs) -> ADD:
        ...
        actor_critic: ActorCriticDiscriminator = actor_critic_class(
            obs, self.cfg["obs_groups"], self.env.num_actions, **self.policy_cfg
        ).to(self.device)

        # initialize the algorithm
        alg_class = eval(self.alg_cfg.pop("class_name"))
        alg: ADD = alg_class(actor_critic, device=self.device, **self.alg_cfg, multi_gpu_cfg=self.multi_gpu_cfg)
